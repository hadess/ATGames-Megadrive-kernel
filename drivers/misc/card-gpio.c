...
#define CARDS_IOC_ENTER_WIRELESS_TEST 	_IOWR(CARDS_IOC_MAGIC, 10, int)
#define CARDS_IOC_GET_WIREPAD_STATUS 	_IOWR(CARDS_IOC_MAGIC, 11, int)


//for gpio test
#define CARDS_IOC_OPERA_ONE_GPIO_WR _IOWR(CARDS_IOC_MAGIC, 120, int)
#define CARDS_IOC_OPERA_BUS_GPIO_WR  _IOWR(CARDS_IOC_MAGIC, 121, int)
...
#define CARD_POWER_EN     49   //GPIO1_C0   //86   //GPIO2_C6
...
extern int enterWirelessTestMode();
...
extern int gpio_set_pin_mux_my(unsigned group, unsigned pin_num);

extern unsigned int wirePadStatus[2];

static unsigned int lastA21 = 0;  //
...
static void initGpioDirection_nomutex();
/***
...
static long cards_ioctl(struct file *filp, unsigned int cmd,
		...
		switch(cmd){
			case CARDS_IOCGETSECTOR:{
				lastA21 = 0xffffffff; //invalid value
				for(i=0;i<256;i++){
			...
			case CARDS_IOCSWITCHKEY:{
				//close card power
				printk("stop card power\n");				
				gpio_bank_set_value(1, 0, (1<< 17));				
			...
			case CARDS_IOCSWITCHCARD:{
				printk("CARDS_IOCSWITCHCARD %d \n", __LINE__);
				cardaddress=0;
				stopScankey();
				printk("CARDS_IOCSWITCHCARD %d \n", __LINE__);
				//initGpio();
				initGpioDirection_nomutex();
				printk("CARDS_IOCSWITCHCARD %d \n", __LINE__);
				//open card power
				gpio_bank_set_value(1, 1 << 17, (1<< 17));
				printk("CARDS_IOCSWITCHCARD %d \n", __LINE__);
				msleep(200);
				printk("CARDS_IOCSWITCHCARD %d \n", __LINE__);
				printk("open card power\n");				
			}
			break;
			case CARDS_IOCSTART_WIRELESSKEY_SCAN:{
				printk("start card power");
				//close card power
				gpio_bank_set_value(1, 0, (1<< 17));				
			...
			break;
			case CARDS_IOC_ENTER_WIRELESS_TEST:{
				return enterWirelessTestMode(arg);
			}
			break;
			case CARDS_IOC_GET_WIREPAD_STATUS:{
				//int status = wirePadStatus[0] | (wirePadStatus[1] << 16);
				
				//modify according to jason
				int status = (wirePadStatus[0] == 3 ? 1 : 0) | ((wirePadStatus[1] == 3 ? 1 : 0) << 16);
...
static int getAddrValue(int tempaddr){
	...
	unsigned int wr_addr_mask = (0x1ffff << 7) | (1u << 25) | (7u << 28);
#endif	
	//gpio_direction_output(CARD_OE,1);
	gpio_bank_set_value(1, 1 << 16, (1<< 16));  //CARD_OE
	...
	if(lastA21 != ((tempaddr>>21)&0x1))//remove no use set_value to speed read.
	{
		gpio_bank_set_value(1, ((tempaddr>>21)&0x1) << 15, (1<< 15));  //CARD_A21
		lastA21 =  ((tempaddr>>21)&0x1);
	}
	//gpio_direction_output(CARD_OE,1);
	...
	gpio_bank_set_value(1, 0 << 16, (1<< 16));  //CARD_OE
	//ndelay(70);
	{volatile int delay; for(delay=0; delay < 0x6; delay++);}
#endif	
...
static void initGpioDirection()
{
	unsigned int wr_addr_mask = (0x1ffff << 7) | (1u << 25) | (7u << 28); //0xffffffff; 
	unsigned int rd_data_mask = (1 << CARD_D0) | (1 << CARD_D1)  | (1 << CARD_D2)  | (1 << CARD_D3)  | (1 << CARD_D4)  | (1 << CARD_D5)  | (1 << CARD_D6)  | (1 << CARD_D7)  | (1 << CARD_D8)  | (1 << CARD_D9)  | (1 << CARD_D10)  | (1 << CARD_D11)  | (1 << CARD_D12)  | (1 << CARD_D13)  | (1 << CARD_D14)  | (1 << CARD_D15)  ;
	...
	gpio_direction_output(CARD_OE,1);
	gpio_direction_output(CARD_POWER_EN,0);
...
/*
	...
	gpio_bank_set_direction(2, (1<< 7), (1<< 7));
	gpio_bank_set_direction(2, (1<< 8), (1<< 8));
	gpio_bank_set_direction(2, (1<< 9), (1<< 9));
	gpio_bank_set_direction(2, (1<< 10), (1<< 10));
	gpio_bank_set_direction(2, (1<< 11), (1<< 11));
	gpio_bank_set_direction(2, (1<< 12), (1<< 12));
	gpio_bank_set_direction(2, (1<< 13), (1<< 13));
	gpio_bank_set_direction(2, (1<< 14), (1<< 14));
	gpio_bank_set_direction(2, (1<< 15), (1<< 15));
	gpio_bank_set_direction(2, (1<< 16), (1<< 16));
	gpio_bank_set_direction(2, (1<< 17), (1<< 17));
	gpio_bank_set_direction(2, (1<< 18), (1<< 18));
	gpio_bank_set_direction(2, (1<< 19), (1<< 19));
	gpio_bank_set_direction(2, (1<< 20), (1<< 20));
	gpio_bank_set_direction(2, (1<< 21), (1<< 21));
	gpio_bank_set_direction(2, (1<< 22), (1<< 22));
	gpio_bank_set_direction(2, (1<< 23), (1<< 23));
	gpio_bank_set_direction(2, (1<< 25), (1<< 25));
	gpio_bank_set_direction(2, (1<< 28), (1<< 28));
	gpio_bank_set_direction(2, (1<< 29), (1<< 29));
	gpio_bank_set_direction(2, (1<< 30), (1<< 30));
	*/
	

	gpio_bank_set_direction(2, wr_addr_mask, wr_addr_mask); // set output
	gpio_bank_set_direction(2, (1<<15), (1<< 15));
	gpio_bank_set_value(1,  0 , (1<<15));
	//gpio_direction_output(CARD_A21,0);
	
	//gpio_direction_output(CARD_OE,1);
	gpio_bank_set_direction(1, (1<< 16), (1<< 16));
	gpio_bank_set_value(1,  1<<16 , (1<<16));
	
	//gpio_direction_output(CARD_POWER_EN,0);
	gpio_bank_set_direction(1, (1<< 17), (1<< 17));
	gpio_bank_set_value(1,  0 , (1<<17));
	...
	gpio_direction_input(CARD_D14);
	gpio_direction_input(CARD_D15);*/
	gpio_set_pin_mux_my(0, 28);
	gpio_bank_set_direction(0, 0, rd_data_mask); // set input
	//gpio_bank_set_direction(0, 0, (1u<<CARD_D15));
...
static void initGpio(){
	...
	gpio_request(CARD_POWER_EN,"CARD_POWER");
