...
/****
llx added for gpio bank setvalue;
value:GPIO_SWPORT_DR寄存器的值，按32位计算
mask: 只改变其中为1的位， 其他位的值不改变。
****/
void rockchip_bank_gpio_set(struct gpio_chip *gc,int value, unsigned int mask)
{
	struct rockchip_pin_bank *bank = gc_to_pin_bank(gc);	
	struct rockchip_pinctrl *info = bank->drvdata;
	void __iomem *reg = bank->reg_base + GPIO_SWPORT_DR;
	...
	//for test
	//void __iomem *direction = bank->reg_base + GPIO_SWPORT_DDR;
	//writel((1u<<26)| readl(direction), direction);  //write direction.
	//printk("rockchip_bank_gpio_set para: %x, %x, %x..\n", value, mask,  readl(direction));
	...
	data &=  ~mask;
	data |= (value & mask);  // avoid user's wrong's 'value'
	writel(data, reg);

	spin_unlock_irqrestore(&bank->slock, flags);
	//printk("rockchip_bank_gpio_set: data = 0x%x \n", data);
	
//	DBG_PINCTRL("%s:GPIO%d-%d level = %d\n", __func__, bank->bank_num, offset1, value);
}

/*set gpio bank direction
 * add by hxc
 *   1  ---> output  0 ----> input; which is in value bits. 
*/
void rockchip_bank_gpio_setdir(struct gpio_chip *gc,int value, unsigned int mask)
{
	struct rockchip_pin_bank *bank = gc_to_pin_bank(gc);	
	struct rockchip_pinctrl *info = bank->drvdata;
	void __iomem *reg = bank->reg_base + GPIO_SWPORT_DDR;
	unsigned long flags;
	u32 data;	
...
void rockchip_gpio_setmux_my(struct gpio_chip *chip, int pinnum)
{
	struct rockchip_pin_bank *bank;
	int pin;
	u32 data, result;
...
EXPORT_SYMBOL_GPL(rockchip_bank_gpio_set);
EXPORT_SYMBOL_GPL(rockchip_bank_gpio_setdir);
...
/****
llx added for gpio bank getvalue;
return vlaue:GPIO_EXT_PORT，按32位计算
****//*
 * Returns the level of the pin for input direction and setting of the DR
 * register for output gpios.
 */
int rockchip_bank_gpio_get(struct gpio_chip *gc)
{
	struct rockchip_pin_bank *bank = gc_to_pin_bank(gc);	
	struct rockchip_pinctrl *info = bank->drvdata;
	u32 data;
	...
	//DBG_PINCTRL("%s:GPIO%d level = %d\n", __func__, bank->bank_num, data);
	return data;
}

EXPORT_SYMBOL(rockchip_bank_gpio_get);
